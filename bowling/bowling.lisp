...
(defgeneric regular-frame-score (game index)
  (:method ((game game) (index integer))
    (sum-of-next-two-balls game index)))

(defgeneric strike-p (game index)
  (:method ((game game) (index integer))
     (= 10 (aref (rolls game) index))))

(defgeneric strike-frame-score (game index)
  (:method ((game game) (index integer))
    (+ 10 (sum-of-next-two-balls game (1+ index)))))

(defgeneric spare-p (game index)
  (:method ((game game) (index integer))
     (= 10 (regular-frame-score game index))))

(defgeneric spare-frame-score (game index)
  (:method ((game game) (index integer))
    (let ((rolls (rolls game)))
      (+ 10 (aref rolls (+ index 2))))))

(defgeneric frame-score (game index)
  (:method ((game game) (index integer))
     (cond
      ((strike-p game index) (values (strike-frame-score game index) 1))
      ((spare-p game index) (values (spare-frame-score game index) 2))
      (t (values (regular-frame-score game index) 2)))))
